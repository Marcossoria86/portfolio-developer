document.addEventListener("DOMContentLoaded", () => {
  ...
  function recalcularPosicionesSecciones() {
    posicionesSecciones = Array.from(seccionesPagina).map((seccion) => ({
      id: seccion.getAttribute("id"),
      top: seccion.offsetTop,
      alto: seccion.clientHeight,
    }));
  }
  ...
  function actualizarNavbar() {
    ...
    let seccionActualId = "";
    posicionesSecciones.forEach(({ id, top, alto }) => {
  ...
  let navbarTickPendiente = false;
  function solicitarActualizarNavbar() {
    if (navbarTickPendiente) return;
    navbarTickPendiente = true;
    requestAnimationFrame(() => {
      actualizarNavbar();
      navbarTickPendiente = false;
    });
  }

  recalcularPosicionesSecciones();
  document.addEventListener("scroll", solicitarActualizarNavbar, {
    passive: true,
  });
  window.addEventListener("resize", recalcularPosicionesSecciones, {
    passive: true,
  });
  actualizarNavbar();
